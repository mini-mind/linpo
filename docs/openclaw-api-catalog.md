# OpenClaw WebSocket API 参考（当前口径）

> 用途：保留 OpenClaw 全量接口清单（含未接入项），并明确 Linpo 当前版本 在后端适配层的真实接入状态。  
> 口径日期：2026-03-27。  
...
- `✅ 已接入（Adapter）`：已进入 `ProviderAdapter/OpenClawAdapter` 主链路，并可被 当前版本 业务调用。
