...
- `POST /api/v1/boards/{board_id}/tasks/flow/generate`：启动或续接一次 planner 增量编辑会话，不直接落看板任务；支持可选 `planner_agent_id/current_nodes/current_edges/planner_session_key` 以在已有流程上增量改图。`planner_agent_id` 由前端实例配置下发，未传时后端回退 `FLOW_DECOMPOSITION_AGENT_ID`。后端先按当前登录用户校验 `instance_id` 归属，并使用该实例上下文发起 planner 会话；随后持久化用户消息、当前工作流快照与 planner session 状态，再把“历史消息 + 当前快照 + 本次需求”发送给 planner。响应至少返回 `planner_session_key` 与当前 draft snapshot，不再要求等待完整整图生成结束。
