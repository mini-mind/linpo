...
| Task | Location | Notes |
|------|----------|-------|
| Frontend host | session-g-ops/deploy/prod/docker-compose.frontend.yml | edge/gateway/web/searxng |
| Worker host | session-g-ops/deploy/worker/docker-compose.yml | playwright-gateway + docker-socket-proxy |
...
docker compose -f session-g-ops/deploy/worker/docker-compose.yml up -d
```
