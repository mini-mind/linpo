...
- `ops/deploy/prod/docker-compose.frontend.yml`: 绑定 `127.0.0.1:8082->80`（用于 frontend host 本机调试）
...
### API 端点
- `/api/health` -> `${API_BACKEND_URL:-api:8000}/health`
  - 健康检查特殊映射 (精确匹配)
  - 返回: `{ "status": "ok" }`

- `/api/` -> `${API_BACKEND_URL:-api:8000}`
  - 所有 `/api/` 前缀请求保留路径并转发到后端
  - 例如: `/api/tasks` -> `http://api:8000/api/tasks`

### WebSocket 端点
- `/ws/` -> `${API_BACKEND_URL:-api:8000}`
  - 所有 `/ws/` 前缀请求保留路径并转发到后端 WebSocket 服务
  - 超时时间: 3600s (1小时)
  - 例如: `/ws/runs/{run_id}` -> `ws://api:8000/ws/runs/{run_id}`
...
- **内部管理**（`/internal/*` 直连后端，不经过 gateway）:
  - 容器内：`http://api:8000/internal/tenants`
