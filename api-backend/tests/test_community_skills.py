...
def test_install_community_skill(tmp_path: pathlib.Path, monkeypatch: _MonkeyPatch) -> None:
...
""".lstrip()
    ...
    install_resp = client.post(
        f"/api/runs/{run_id}/agents/{root_agent_id}/skills/install",
        json={"skill_key": "hello_world"},
        headers={"X-API-Key": api_key},
    )
    assert install_resp.status_code == 200
    payload = install_resp.json()
    skills = cast(list[dict[str, object]], payload["skills"])
    assert any(skill.get("filename") == "hello_world.py" for skill in skills)
    ...
    skills_out = cast(list[dict[str, object]], get_resp.json()["skills"])
    assert any(skill.get("filename") == "hello_world.py" for skill in skills_out)
    ...
    assert (agent_root / "skills" / "hello_world.py").is_file()
