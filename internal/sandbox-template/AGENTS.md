...
## STRUCTURE
```
session-e-internal/sandbox-template/
├── app/              # FastAPI app
├── tests/            # pytest tests
├── requirements.txt
...
| Task | Location | Notes |
|------|----------|-------|
| API handlers | session-e-internal/sandbox-template/app/main.py | `/templates/skill` + `/health` |
| Tests | session-e-internal/sandbox-template/tests/ | pytest `test_*.py` |
...
## COMMANDS
```bash
cd session-e-internal/sandbox-template
. .venv/bin/activate
python -m pytest -q
```
