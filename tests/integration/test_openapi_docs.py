...
TASKS_FLOW_RUNTIME_WRITE_ENDPOINTS: tuple[tuple[str, str], ...] = (
    ...
    ("/api/v1/boards/{board_id}/tasks/flow/drafts", "post"),
    ("/api/v1/boards/{board_id}/tasks/flow/drafts/{flow_id}", "delete"),
    ...
    ("/api/v1/boards/{board_id}/tasks/flow/planner-sessions/{session_key}/events", "post"),
...
def test_openapi_paths_are_versioned_and_grouped_by_domain() -> None:
    ...
    assert (
        cast(dict[str, Any], paths["/api/v1/boards/{board_id}/tasks/flow/planner-sessions/{session_key}/events"])[
            "post"
        ]["tags"]
        == ["flow-internal"]
    )
