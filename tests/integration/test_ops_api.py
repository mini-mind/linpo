...
def test_ops_setup_ready_when_required_configs_exist_and_instance_bound(
    ...
    monkeypatch.setenv("LINPO_SECRET_ENCRYPTION_KEY", "ready-secret-key")
...
def test_ops_diagnostics_ready_when_all_checks_pass(
    ...
    monkeypatch.setenv("LINPO_DATABASE_URL", isolated_database_url)
    monkeypatch.setenv("LINPO_SECRET_ENCRYPTION_KEY", "diag-secret-key")
    monkeypatch.setenv("OPENCLAW_BASE_URL", "ws://ops.example:28789")
    monkeypatch.setenv("OPENCLAW_GATEWAY_TOKEN", "token-openclaw")
    monkeypatch.setenv("OPENCLAW_ORIGIN", "http://ops.example:28789")
    monkeypatch.setenv("FLOW_DECOMPOSITION_OPENCLAW_BASE_URL", "ws://ops.example:38789")
...
def test_ops_diagnostics_reports_recent_instance_degraded_context_when_checks_pass(
    monkeypatch: pytest.MonkeyPatch,
    isolated_database_url: str,
    auth_cookie: str,
    db_handle: Session,
    ...
    db_handle.add_all(
        [
            Instance(
                ...
                name="ops-instance-ok",
                type="openclaw",
                endpoint="http://198.51.100.30:28789",
                gateway_token_enc="enc",
                status="active",
            ),
            Instance(
                user_id=user.id,
                name="ops-instance-degraded",
                type="openclaw",
                endpoint="http://198.51.100.31:28789",
                gateway_token_enc="enc",
                status="failed",
            ),
        ]
    ...
    assert recent_error["checkKey"] == "instance_connectivity_degraded"
    assert "ops-instance-degraded(status=failed)" in recent_error["message"]
