...
def test_websocket_supports_openclaw_session_messages_with_delta_content_shapes(
    monkeypatch: Any,
) -> None:
    session_key = "agent:main:main"
    ...
    assert payloads[1]["payload"]["messages"] == [
        {"role": "assistant", "text": "hello"},
    ]
    assert payloads[1]["payload"]["update_mode"] == "append_chunk"


def test_websocket_normalizes_agent_prefixed_flow_session_key(
    monkeypatch: Any,
) -> None:
    planner_session_key = "linpo:flow:default:planner:planner-default:prefixed"
    upstream_prefixed_session_key = f"agent:main:{planner_session_key}"
                    ...
                    "snapshot": {
                        "health": {
                            ...
                            "agents": [
                                {
                                    "agentId": "main",
                                    "sessions": {"recent": [{"key": upstream_prefixed_session_key, "updatedAt": 1773632400000}]},
                                }
                            ],
                        },
                        "presence": [],
            ...
            on_message(
                {
                    ...
                    "event": "agent",
                    "payload": {
                        "runId": "run-prefixed-1",
                        "stream": "assistant",
                        "sessionKey": upstream_prefixed_session_key,
                        "data": {"text": "hello prefixed session"},
                    },
                }
            )
    ...
    messages = websocket(
        ...
        messages=[
            {
                "type": "subscribe",
                "channel": observer_data.session_messages_channel(planner_session_key),
            }
        ],
        idle_hooks=[lambda: None, lambda: None],
    )
    ...
    assert payloads[1]["channel"] == observer_data.session_messages_channel(planner_session_key)
    assert payloads[1]["payload"]["session_key"] == planner_session_key
    assert payloads[1]["payload"]["messages"] == [
        {"role": "assistant", "text": "hello prefixed session"},
    ]
