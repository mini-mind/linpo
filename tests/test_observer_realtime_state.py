...
def test_get_observer_data_source_rebuilds_when_adapter_config_changes_even_with_same_external_cache_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    creations = 0
    ...
    class FakeAdapter:
        def __init__(self) -> None:
            self._config_key = ("ws://127.0.0.1:28789", "token-one", "http://127.0.0.1:28789")
    ...
    adapter = FakeAdapter()
    first = observer_data.get_observer_data_source(
        "openclaw",
        adapter=cast(Any, adapter),
        cache_key=("shared-cache-key",),
    )
    first_reuse = observer_data.get_observer_data_source(
        "openclaw",
        adapter=cast(Any, adapter),
        cache_key=("shared-cache-key",),
    )
    adapter._config_key = ("ws://127.0.0.1:28789", "token-two", "http://127.0.0.1:28789")
    second = observer_data.get_observer_data_source(
        "openclaw",
        adapter=cast(Any, adapter),
        cache_key=("shared-cache-key",),
    )
    second_reuse = observer_data.get_observer_data_source(
        "openclaw",
        adapter=cast(Any, adapter),
        cache_key=("shared-cache-key",),
    )

    assert first_reuse is first
    assert second is not first
    assert second_reuse is second
    assert creations == 2
