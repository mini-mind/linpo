...
from app.services.flow_draft_service import FlowDraftRevisionConflictError, FlowDraftService
...
def test_upsert_flow_draft_rejects_stale_revision_for_existing_record() -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:")
    Base.metadata.create_all(engine)
    service = FlowDraftService()

    with Session(engine) as db_session:
        user = User(username="carl", email="carl@example.com", password_hash="hash")
        db_session.add(user)
        db_session.commit()
        db_session.refresh(user)
        ...
        latest = _build_payload()
        latest.name = "最新版本"
        latest.revision = 0
        updated = service.upsert_flow_draft(
            db_session,
            user_id=user.id,
            board_id="default",
            payload=latest,
        ...
        stale = _build_payload()
        stale.name = "过期覆盖"
        stale.revision = 0
        with pytest.raises(FlowDraftRevisionConflictError):
            service.upsert_flow_draft(
                db_session,
                user_id=user.id,
                board_id="default",
                payload=stale,
