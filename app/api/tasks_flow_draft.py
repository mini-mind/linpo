...
from app.services.flow_draft_service import (
    FlowDraftRevisionConflictError,
    FlowDraftService,
...
def _to_flow_draft_item(draft: FlowDraft) -> FlowDraftItem:
    ...
    return FlowDraftItem(
        ...
        planner_runtime=FlowDraftPlannerRuntimeItem.model_validate(
            draft.planner_runtime if isinstance(draft.planner_runtime, dict) else {}
        ),
...
@router.post(
    "/flow/drafts",
    response_model=FlowDraftItem,
    responses={409: {"description": "flow draft revision conflict"}},
    tags=["flow"],
)
def upsert_flow_draft(
    ...
    except FlowDraftRevisionConflictError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="flow draft revision conflict") from exc
