...
from app.services.task_output_service import (
    ...
    task_output_sandbox_roots as task_output_sandbox_roots_service,
...
_TASK_OUTPUT_SANDBOX_ROOTS = task_output_sandbox_roots_service()
...
def task_output_sandbox_roots() -> tuple[Path, ...]:
    return _TASK_OUTPUT_SANDBOX_ROOTS
...
def is_task_output_sandbox_path(path: Path) -> bool:
    for root in _TASK_OUTPUT_SANDBOX_ROOTS:
        try:
...
def extract_output_paths_from_artifact(item: str) -> list[str]:
    ...
    if lowered.startswith('artifact:'):
        ...
        if value.startswith('/') or value.startswith('~/'):
            ...
            if match is not None:
                ...
                tilde_match = re.match(tilde_path_pattern, value)
                if tilde_match is not None:
                    add_candidate(tilde_match.group(1))
    ...
    if normalized.startswith('/'):
        ...
        tilde_match = re.match(tilde_path_pattern, normalized)
        if tilde_match is not None:
            add_candidate(tilde_match.group(1))

    for match in re.findall(r"(/(?:tmp|home|root)/[^\s\"'<>]+)", normalized):
        add_candidate(match)
    for match in re.findall(r"(~/[^\s\"'<>]+)", normalized):
        add_candidate(match)
