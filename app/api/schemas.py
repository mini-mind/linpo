...
class InstanceTokenUsageSample(_CommonCamelResponseModel):
    date: str
    input_tokens: int
    output_tokens: int
    total_tokens: int
...
class InstanceTokenUsageResponse(_CommonCamelResponseModel):
    days: int
    daily: list[InstanceTokenUsageSample]
...
class InstanceFileWriteRequest(_CommonCamelRequestModel):
    task_id: str | None = Field(default=None, alias="taskId")
    board_id: str = Field(default="default", alias="boardId")
    path: str | None = None
    content: str = Field(default="", max_length=2_000_000)


class InstanceFileWriteResponse(_CommonCamelResponseModel):
    path: str
    size_bytes: int
    updated_at: str
    exists: bool
...
class InstanceFileDeleteResponse(_CommonCamelResponseModel):
    path: str
    deleted: bool
    updated_at: str
...
class FlowDraftPlannerRuntimeItem(_TasksFlowResponseModel):
    planner_session_status: FlowPlannerSessionStatus | Literal["idle"] = "idle"
    is_planning: bool = False
    is_planner_stopping: bool = False
    is_overlay_close_blocked: bool = False
...
class FlowDraftItem(_TasksFlowResponseModel):
    ...
    planner_runtime: FlowDraftPlannerRuntimeItem = Field(default_factory=FlowDraftPlannerRuntimeItem)
    revision: int
    created_at: str
    updated_at: str
...
class FlowDraftPlannerRuntimeRequest(FlowDraftPlannerRuntimeItem):
    model_config = ConfigDict(alias_generator=_to_camel_case, populate_by_name=False)
...
class FlowDraftUpsertRequest(_TasksFlowRequestModel):
    ...
    execution_session_prefix: str | None = None
    executor_agent_id: str | None = None
    planner_runtime: FlowDraftPlannerRuntimeRequest = Field(default_factory=FlowDraftPlannerRuntimeRequest)
...
FlowPlannerCallbackEventType = Literal[
    "assistant_delta",
    "assistant_message",
    "tool_call_start",
    "tool_call_delta",
...
class FlowPlannerSessionEventRequest(_TasksFlowRequestModel):
    type: FlowPlannerCallbackEventType | str = "message"
    role: FlowChatRole | None = None
    content: str | None = None
    kind: str | None = Field(default=None, max_length=64)
    payload: dict[str, Any] = Field(default_factory=dict)
    status: FlowPlannerSessionStatus | None = None
    error: str | None = None
    detail: str | None = None
    created_at: str | None = None


class FlowPlannerSessionEventsRequest(_TasksFlowRequestModel):
    events: list[FlowPlannerSessionEventRequest] = Field(default_factory=list)


class FlowPlannerSessionEventsResponse(_TasksFlowResponseModel):
    session_key: str
    status: FlowPlannerSessionStatus
    revision: int
