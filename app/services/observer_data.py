...
class OpenClawObserverDataSource(StateBackedObserverDataSource):
    ...
    def _normalize_realtime_session_key(self, value: object) -> str | None:
        ...
        # OpenClaw 某些 realtime 事件会把 planner session 包成 `agent:<id>:<planner_session_key>`，
        # 这里统一折叠成规范的 `linpo:flow:...`，确保后续 channel 匹配。
        if marker_index > 0:
            normalized = normalized[marker_index:]
        return normalized

    def _extract_assistant_text_chunks(self, value: object) -> list[str]:
        chunks: list[str] = []
        self._collect_assistant_text_chunks(value, chunks)
        return chunks

    def _collect_assistant_text_chunks(self, value: object, chunks: list[str]) -> None:
        ...
        if isinstance(value, list):
            for item in value:
                self._collect_assistant_text_chunks(item, chunks)
            return
        if not isinstance(value, dict):
            return
        # 兼容 OpenClaw 上游可能变更的增量字段，同时避免同一事件多字段重复导致文本翻倍。
        # 优先使用最细粒度字段（delta），命中后直接返回，不再继续读取其它同级字段。
        for key in ("delta", "text", "chunk", "message", "output_text"):
            candidate = value.get(key)
            before_count = len(chunks)
            self._collect_assistant_text_chunks(candidate, chunks)
            if len(chunks) > before_count:
                return
        for key in ("content", "parts", "fragments", "items", "segments"):
            nested = value.get(key)
            before_count = len(chunks)
            self._collect_assistant_text_chunks(nested, chunks)
            if len(chunks) > before_count:
                return

    def _map_realtime_message(self, message: dict[str, Any]) -> ObserverRealtimeEvent | None:
        if message.get("type") != "event":
            logger.debug("[observer.realtime] skip_non_event type=%s", message.get("type"))
            return None

        event_name = message.get("event")
        logger.debug("[observer.realtime] received event=%s", event_name)
        payload = self._require_dict(
            message.get("payload"),
            detail=f"OpenClaw realtime event {event_name} missing payload",
        )
        if event_name == "chat":
            logger.info("[observer.realtime] mapped_chat_control_event")
            return ObserverRealtimeEvent(
                type="control_request_updated",
        ...
        if event_name == "agent":
            ...
            session_key = self._normalize_realtime_session_key(payload.get("sessionKey"))
            ...
            logger.debug(
                "[observer.realtime] agent_stream_event stream=%s run_id=%s session_key=%s",
                stream,
                run_id,
                session_key,
            )
            if stream == "lifecycle" and isinstance(run_id, str):
                ...
                if (
                    (not isinstance(resolved_session_key, str) or not resolved_session_key)
                    and isinstance(run_id, str)
                ):
                    resolved_session_key = self._session_key_by_run_id.get(run_id)
                if phase in {"end", "error"}:
                    self._session_key_by_run_id.pop(run_id, None)
                if isinstance(resolved_session_key, str) and resolved_session_key and phase in {"end", "error"}:
                    logger.info(
                        "[observer.realtime] lifecycle_terminal_emitted run_id=%s session_key=%s phase=%s",
                        run_id,
            ...
            if normalized_stream not in {"assistant", "assistant_delta", "delta", "response", "message"}:
                # 关键排查日志：流程规划链路若缺少 assistant 增量，这里能看到上游到底发了什么 stream 类型。
                logger.info(
                    "[observer.realtime] skip_non_assistant_stream stream=%s session_key=%s run_id=%s data_keys=%s",
                    stream,
                    resolved_session_key,
                    run_id,
            ...
            if not isinstance(resolved_session_key, str) or not resolved_session_key:
                logger.warning(
                    "[observer.realtime] skip_assistant_missing_session run_id=%s session_key=%s",
                    run_id,
                    session_key,
                )
                return None
            ...
            if not isinstance(data, dict):
                logger.debug(
                    "[observer.realtime] skip_assistant_invalid_data session_key=%s run_id=%s data_type=%s",
                    resolved_session_key,
                    run_id,
                    type(data).__name__,
                )
            ...
            text_chunks = self._extract_assistant_text_chunks(data)
            if not text_chunks:
                logger.debug(
                    "[observer.realtime] skip_assistant_empty_text session_key=%s run_id=%s data_keys=%s",
                    resolved_session_key,
                    run_id,
                    sorted(data.keys()),
                )
            ...
            logger.info(
                "[observer.realtime] assistant_delta_emitted session_key=%s run_id=%s chunk_count=%s text_len=%s",
                resolved_session_key,
                run_id,
                len(text_chunks),
                sum(len(item) for item in text_chunks),
            )
            return ObserverRealtimeEvent(
                type="session_messages_updated",
                session_key=resolved_session_key,
                messages=[{"role": "assistant", "text": text} for text in text_chunks],
...
def _resolved_openclaw_cache_key(
    *,
    adapter: ProviderAdapter | None,
    cache_key: object | None,
) -> object:
    if adapter is None:
        return cache_key or _openclaw_data_source_config()
    adapter_key = adapter.config_key()
    if cache_key is None:
        return adapter_key
    return (adapter_key, cache_key)
...
def get_observer_data_source(
    ...
    *,
    ...
    if selected == "openclaw":
        resolved_cache_key = _resolved_openclaw_cache_key(
            adapter=adapter,
            cache_key=cache_key,
        )
