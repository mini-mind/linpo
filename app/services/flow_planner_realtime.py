...
class FlowPlannerRealtimeHub:
    ...
    def publish_messages_updated(
        ...
        *,
        ...
        normalized_update_mode = update_mode.strip() if isinstance(update_mode, str) else "replace"
        if normalized_update_mode not in {"replace", "append_chunk"}:
            normalized_update_mode = "replace"
        payload: dict[str, Any] = {
            "session_key": session_key,
            "messages": messages,
            "update_mode": normalized_update_mode,
        }
        if normalized_update_mode == "append_chunk" and append_chunk:
            payload["append_chunk"] = append_chunk
        self._publish(
            user_id=user_id,
            board_id=board_id,
            session_key=session_key,
