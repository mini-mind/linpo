...
def test_agent_sources_roundtrip(tmp_path: pathlib.Path, monkeypatch: _MonkeyPatch) -> None:
    ...
    payload = {"sources": [{"path": "docs/manuals", "label": "Manuals"}]}
    put_resp = client.put(
        f"/api/runs/{run_id}/agents/{root_agent_id}/sources",
        json=payload,
        headers={"X-API-Key": api_key},
    )
    ...
    assert (agent_root / "context" / "sources" / "docs" / "manuals").is_dir()
