...
| Task | Location | Notes |
|------|----------|-------|
| API handlers | session-d-browser/worker-playwright/app/main.py | `/run` + `/health` |
| Health tests | session-d-browser/worker-playwright/tests/test_health.py | Minimal smoke test |
...
## COMMANDS
```bash
cd session-d-browser/worker-playwright
. .venv/bin/activate
python -m pytest -q
```
