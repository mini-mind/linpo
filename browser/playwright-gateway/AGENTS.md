...
| Task | Location | Notes |
|------|----------|-------|
| Runner orchestration | session-d-browser/playwright-gateway/app/main.py | `/run` handler and Docker logic |
| Health tests | session-d-browser/playwright-gateway/tests/test_health.py | Minimal smoke test |
...
## COMMANDS
```bash
cd session-d-browser/playwright-gateway
. .venv/bin/activate
python -m pytest -q
```
