...
describe('usePlannerOperationQueue', () => {
  ...
  it('clears only the specified session queue runtime without affecting other sessions', () => {
    const applyDraftNodes = vi.fn();
    let enqueue: ((sessionKey: string, operations: any[]) => void) | null = null;
    let clearRuntime: ((sessionKey: string) => void) | null = null;
    ...
    function Probe(): null {
      ...
      enqueue = runtime.enqueuePlannerOperations;
      clearRuntime = runtime.clearPlannerOperationQueueRuntime as unknown as (sessionKey: string) => void;
      return null;
    }
    ...
    act(() => {
      ...
      enqueue?.('session-b', [{
        ...
        node: {
          id: 'node-new',
          title: 'new session node',
          description: '',
          depends_on: [],
          sensitive: false,
        },
      }]);
      clearRuntime?.('session-a');
    });
    ...
    expect(applyDraftNodes).toHaveBeenCalledTimes(1);
    const appliedSession = (applyDraftNodes.mock.calls[0] as [FlowPlannerNodeDraft[], string])[1];
    const appliedNodes = (applyDraftNodes.mock.calls[0] as [FlowPlannerNodeDraft[], string])[0].map((item) => item.id);
    expect(appliedSession).toBe('session-b');
    expect(appliedNodes).toContain('node-new');
    expect(appliedNodes).not.toContain('node-old');
  });

  it('clears only target session queue runtime and keeps other session operations', () => {
    const applyDraftNodes = vi.fn();
    let enqueue: ((sessionKey: string, operations: any[]) => void) | null = null;
    let clearRuntime: ((sessionKey?: string) => void) | null = null;
    ...
    act(() => {
      ...
      // 只清理旧会话运行态，新会话队列仍应可继续消费。
      clearRuntime?.('session-old');
    });
    ...
    expect(applyDraftNodes).toHaveBeenCalledTimes(1);
    const [draftNodes, sessionKey] = applyDraftNodes.mock.calls[0] as [FlowPlannerNodeDraft[], string];
    expect(sessionKey).toBe('session-new');
    expect(draftNodes.map((item) => item.id)).toEqual(['node-a', 'node-new-1']);
  });
  ...
  it('clears only target session runtime so old-session queue does not clear new-session queue', () => {
    const applyDraftNodes = vi.fn();
    let enqueue: ((sessionKey: string, operations: any[]) => void) | null = null;
    let clearRuntime: ((sessionKey?: string) => void) | null = null;
    ...
    function Probe(): null {
      ...
      enqueue = runtime.enqueuePlannerOperations;
      clearRuntime = runtime.clearPlannerOperationQueueRuntime as (sessionKey?: string) => void;
      return null;
    }
    ...
    const [draftNodes, sessionKey] = applyDraftNodes.mock.calls[0] as [FlowPlannerNodeDraft[], string];
    expect(sessionKey).toBe('session-new');
    expect(draftNodes.some((item) => item.id === 'node-old')).toBe(false);
    expect(draftNodes.some((item) => item.id === 'node-new')).toBe(true);
  });

  it('drops old session pending snapshot so it cannot be consumed after session switch', () => {
    const applyDraftNodes = vi.fn();
    let enqueue: ((sessionKey: string, operations: any[]) => void) | null = null;
    let applyOrQueueSnapshot: ((sessionKey: string, revision: number, nodes: FlowPlannerNodeDraft[]) => boolean) | null = null;
    let clearRuntime: ((sessionKey?: string) => void) | null = null;
    ...
    act(() => {
      ...
      const queued = applyOrQueueSnapshot?.('session-old', 3, [{
        id: 'node-old-snapshot',
        title: 'Old Snapshot',
        description: '',
        depends_on: [],
        sensitive: false,
