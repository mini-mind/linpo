...
describe('useBlockedFlowAutoSync', () => {
  ...
  it('coalesces changed signatures while first sync is in-flight and only sends latest follow-up', async () => {
    let resolveFirstSync: (() => void) | undefined;
    const syncRequirement = vi.fn()
    ...
    rerender(
      <Probe
        {...baseProps}
        flowNodes={[{ ...baseProps.flowNodes[0], title: 'node-a-v2' }]}
      />
    );
    await act(async () => {
      await Promise.resolve();
    ...
    rerender(
      <Probe
        {...baseProps}
        flowNodes={[{ ...baseProps.flowNodes[0], title: 'node-a-v3' }]}
      />
    );
    await act(async () => {
      await Promise.resolve();
    ...
    const resolveFirstSyncFn = resolveFirstSync;
    if (resolveFirstSyncFn) {
      await act(async () => {
        resolveFirstSyncFn();
        await Promise.resolve();
        await Promise.resolve();
    ...
    expect(syncRequirement).toHaveBeenCalledTimes(2);
    expect(syncRequirement.mock.calls[1]?.[1]).toEqual(
      expect.objectContaining({
        nodes: [expect.objectContaining({ title: 'node-a-v3' })],
      })
    );
    expect(refreshFlowTasks).toHaveBeenCalledTimes(2);
  });

  it('does not resync unchanged signature after runtime toggles between running and blocked', async () => {
    const syncRequirement = vi.fn().mockResolvedValue(undefined);
    const refreshFlowTasks = vi.fn().mockResolvedValue(undefined);
    const addToast = vi.fn();
    const baseProps: ProbeProps = {
    ...
    rerender(<Probe {...baseProps} flowRuntimeState="running" />);
    act(() => {
      vi.advanceTimersByTime(400);
    });
    expect(syncRequirement).toHaveBeenCalledTimes(1);

    rerender(<Probe {...baseProps} flowRuntimeState="blocked" />);
    act(() => {
      vi.advanceTimersByTime(300);
    });
    await Promise.resolve();
