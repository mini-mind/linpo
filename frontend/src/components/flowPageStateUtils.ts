...
export function areFlowDraftRecordsEquivalent(left: FlowDraftRecord, right: FlowDraftRecord): boolean {
  return JSON.stringify({
    ...
    planner_runtime: left.planner_runtime ?? null,
  }) === JSON.stringify({
    id: right.id,
    name: right.name,
    ...
    planner_runtime: right.planner_runtime ?? null,
  });
}
...
export function resolveExecutorAgentId(
  ...
  const mainAgentId = agents.find((agent) => agent.agent_id.trim() === 'main')?.agent_id.trim() ?? '';
  if (mainAgentId) {
    return mainAgentId;
  }
...
export function areFlowChatMessagesEqual(left: FlowChatMessageItem[], right: FlowChatMessageItem[]): boolean {
  ...
  for (let index = 0; index < left.length; index += 1) {
    ...
    if (
      ...
      JSON.stringify(current.payload ?? null) !== JSON.stringify(next.payload ?? null)
    ) {
      return false;
    }
  }
