...
type SidebarAgentDocItem = {
  id: string;
  title: string;
  iconKind: import('./types').FileIconKind;
  ariaLabel: string;
};

type InstanceFilesSidebarProps = {
  isMobile: boolean;
  ...
  renderAgentDocIcon: (kind: import('./types').FileIconKind) => JSX.Element;
...
export function InstanceFilesSidebar({
  ...
  renderAgentDocIcon,
  taskResourceCount,
}: InstanceFilesSidebarProps): JSX.Element {
  return (
    <aside
                ...
                <div key={group.id} style={agentGroupStyle}>
                  ...
                  >
                    <span style={treeLeafNameWrapStyle}>
                      {isCollapsed ? <ChevronRight size={12} aria-hidden="true" style={{ flexShrink: 0, color: '#64748b' }} /> : <ChevronDown size={12} aria-hidden="true" style={{ flexShrink: 0, color: '#64748b' }} />}
                      <Folder size={14} aria-hidden="true" style={{ flexShrink: 0, color: '#64748b' }} />
                      <span style={treeLeafNameStyle}>{group.label}</span>
                    </span>
                  </button>
                  {!isCollapsed ? (
                    <div style={agentGroupChildrenStyle}>
                      {group.docs.map((doc: SidebarAgentDocItem) => {
                        ...
                        return (
                          ...
                          >
                            <span style={treeLeafNameWrapStyle}>
                              {renderAgentDocIcon(doc.iconKind)}
