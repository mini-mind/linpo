...
describe('CollabPage', () => {
  ...
  it('renders desktop sidebar controls with pending-confirmation column action and no top toolbar', async () => {
    mockGetAggregateOverview.mockResolvedValue(buildOverview({ agents: [buildAgent()] }));

    await renderBoard();

    // 桌面端改为侧栏后，不应再出现顶部 toolbar 容器。
    ...
    const stats = screen.getByLabelText('看板统计');
    const viewModeSelect = screen.getByLabelText('分列方式');
    let sidebarContainer = viewModeSelect.parentElement;
    while (sidebarContainer && !sidebarContainer.contains(stats)) {
      sidebarContainer = sidebarContainer.parentElement;
    }
    expect(sidebarContainer).not.toBeNull();

    // 关键控制项应位于侧栏容器内，避免回归到顶部工具栏布局。
    expect(within(sidebarContainer as HTMLElement).getByLabelText('看板统计')).toBeInTheDocument();
    expect(within(sidebarContainer as HTMLElement).getByLabelText('分列方式')).toBeInTheDocument();
  ...
  it('shows task id and requirement_id on task cards for quick reconciliation', async () => {
    ...
    expect(await screen.findByText('任务ID：task-reconcile-1')).toBeInTheDocument();
    expect(await screen.findByText('requirement_id：req-reconcile-1')).toBeInTheDocument();
  });
