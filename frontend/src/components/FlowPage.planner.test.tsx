...
import * as flowPlanGenerationUtils from './flowPlanGenerationUtils';
import {
  renderFlowPage,
...
describe('FlowPage planner', () => {
  ...
  it('switches send button to stop and calls planner stop api while planning', async () => {
    ...
    expect(mockStopFlowPlannerSession).toHaveBeenCalledWith(
      { planner_session_key: expect.stringContaining('linpo:flow:default:planner:') },
      undefined,
      'default'
    );
  ...
  it('keeps stop button when planner session is still planning even if request flag is transiently false', async () => {
    mockGenerateFlowFromRequirement.mockImplementation(() => new Promise<FlowGenerateResponse>(() => {}));
    const originalResolvePlanSubmissionIntent = flowPlanGenerationUtils.resolvePlanSubmissionIntent;
    const resolvePlanSubmissionIntentSpy = vi
      .spyOn(flowPlanGenerationUtils, 'resolvePlanSubmissionIntent')
      .mockImplementation((params) => {
        const originalIntent = originalResolvePlanSubmissionIntent(params);
        return {
          ...originalIntent,
          isPlanning: false,
          plannerSessionStatus: 'planning',
        } as unknown as ReturnType<typeof flowPlanGenerationUtils.resolvePlanSubmissionIntent>;
      });

    try {
      const flowId = seedDraftFlow('draft-planner-button-status-self-heal');
      renderFlowPage(`/flow/edit/${flowId}`);
      await waitForFlowCanvasReady();

      await submitPlannerInstruction('保持 planning 会话');
      ...
      resolvePlanSubmissionIntentSpy.mockRestore();
    }
  });
});
