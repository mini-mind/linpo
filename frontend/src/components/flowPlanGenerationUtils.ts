...
export type PlanFailureGuardIntent =
  | {
      ok: true;
      reason: 'ok';
      pendingRequest: PendingPlannerRequestState;
...
export function resolvePlanSubmissionIntent({
  ...
  return {
    ...
    pendingRequest: {
      ...
      // breaking: 纯事件回调流模式下，画布只由实时事件驱动，不再从 HTTP generate 响应回填。
      allowHttpGraphHydrate: false,
      baselineRevision: currentRevision,
      latestRealtimeRevision: currentRevision,
    },
    isPlanning: true,
    isPlannerStopping: false,
    plannerSessionStatus: 'planning',
...
export function resolvePlanFailureGuardIntent({
  flowScopeAtRequest,
  activeFlowScope,
  pendingRequest,
  requestId,
  plannerSessionKey,
}: ResolvePlanSuccessGuardIntentParams): PlanFailureGuardIntent {
  if (flowScopeAtRequest !== activeFlowScope) {
    return {
      ok: false,
      reason: 'scope_mismatch',
...
export function canHydratePlannerGraphFromHttp(params: {
  ...
  void params;
  // breaking 约束：planner 画布只信任 realtime 事件，HTTP 响应不再参与节点回填。
  return false;
}
