...
function parseFlowPlannerRealtimeMessage(raw: string): FlowPlannerRealtimeMessage {
  ...
  if (type === 'planner_messages_updated') {
    ...
    const updateModeValue = 'update_mode' in payload ? payload.update_mode : payload.updateMode;
    if (
      updateModeValue !== undefined &&
      updateModeValue !== 'replace' &&
      updateModeValue !== 'append_chunk'
    ) {
      throw new Error('Invalid flow planner realtime message');
    }
    return {
      ...
      payload: {
        ...
        messages,
        update_mode:
          updateModeValue === 'replace' || updateModeValue === 'append_chunk'
            ? updateModeValue
            : undefined,
      },
    };
