...
function encodeChatMessage(message: FlowChatMessageItem): UnknownRecord {
  const encoded: UnknownRecord = {
    role: message.role,
    content: message.content,
    createdAt: message.created_at,
  };
  if (typeof message.kind === 'string' && message.kind.trim() !== '') {
    encoded.kind = message.kind.trim();
  }
...
function normalizeFlowChatMessageRole(role: string): FlowChatMessageItem['role'] | null {
  if (role === 'user' || role === 'assistant' || role === 'system') {
    return role;
  }
  if (role === 'tool' || role === 'other') {
    return 'system';
  }
  return null;
}
...
function decodeFlowChatMessageValue(value: unknown): FlowChatMessageItem | null {
  ...
  const rawRole = readRequiredString(record, 'role', 'role');
  const role = rawRole ? normalizeFlowChatMessageRole(rawRole) : null;
  const content = readString(record, 'content', 'content') ?? '';
  const createdAt = readRequiredString(record, 'created_at', 'createdAt');
  if (!role || !createdAt) {
    return null;
  }
  const rawKind = readString(record, 'kind', 'kind');
  const kind = rawKind && rawKind.trim() ? rawKind.trim() : undefined;
  const rawPayload = readField(record, 'payload', 'payload');
  const payload = asRecord(rawPayload);
  const decoded: FlowChatMessageItem = {
    role,
    content,
    created_at: createdAt,
  };
...
export function encodeFlowDraftUpsertRequest(payload: FlowDraftUpsertRequest): UnknownRecord {
  const plannerRuntime = payload.planner_runtime ?? {
  ...
  return {
    ...
    plannerRuntime: {
      plannerSessionStatus: plannerRuntime.planner_session_status,
      isPlanning: plannerRuntime.is_planning,
      isPlannerStopping: plannerRuntime.is_planner_stopping,
      isOverlayCloseBlocked: plannerRuntime.is_overlay_close_blocked,
    },
...
function decodeFlowDraftPlannerRuntime(value: unknown): FlowDraftPlannerRuntimeItem {
  const record = asRecord(value);
  if (!record) {
    return {
      planner_session_status: 'idle',
  ...
  const rawStatus = readRequiredString(record, 'planner_session_status', 'plannerSessionStatus');
  const plannerSessionStatus: FlowDraftPlannerRuntimeItem['planner_session_status'] = (
    rawStatus === 'planning'
    || rawStatus === 'completed'
    || rawStatus === 'stopped'
    || rawStatus === 'failed'
  ...
  return {
    planner_session_status: plannerSessionStatus,
    is_planning: readField(record, 'is_planning', 'isPlanning') === true,
    is_planner_stopping: readField(record, 'is_planner_stopping', 'isPlannerStopping') === true,
    is_overlay_close_blocked: readField(record, 'is_overlay_close_blocked', 'isOverlayCloseBlocked') === true,
  };
}
...
export function decodeFlowDraftItem(value: unknown): FlowDraftItem | null {
  ...
  const createdAt = readRequiredString(record, 'created_at', 'createdAt');
  const updatedAt = readRequiredString(record, 'updated_at', 'updatedAt');
  const revisionValue = readField(record, 'revision', 'revision');
  const revision = typeof revisionValue === 'number' && Number.isFinite(revisionValue) ? revisionValue : 0;
  ...
  return {
    ...
    planner_runtime: decodeFlowDraftPlannerRuntime(readField(record, 'planner_runtime', 'plannerRuntime')),
    revision,
    created_at: createdAt,
    updated_at: updatedAt,
  };
